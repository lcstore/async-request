...
Channel.prototype.channelKey = function () {
    ...
    var channelKey =  KEY_PREIFIX 
			    	 ...
			    	 + KEY_SPLITOR + proxyutils.ip2long(self.host)
    self.key = channelKey
    return self.key
}
...
Channel.prototype.close = function (state,callback) {
	var self = this
  self._useState = STATE_CLOSE
  self.emit('close')
  return self.doCallBack(null,callback)
...
Channel.prototype.toKVProxy = function (callback) {
  ...
  var sHostNum = proxyutils.ip2long(self.host)
