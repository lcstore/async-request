...
Channel.prototype.isUseTooFast  = function() {
  var self = this
  var nowMills = new Date().getTime()
  return self.atime + USE_INTEVAL > nowMills
}

Channel.prototype.isUseTooLong  = function() {
  var self = this
  var nowMills = new Date().getTime()
  return self._firstUseTime + USE_NAX_LONG > nowMills
}

Channel.prototype.isUseCountLimit  = function() {
  var self = this
  return self._useCount > USE_MAX_COUNT
}
...
Channel.prototype.close = function (callback) {
	var self = this
  self._useState = STATE_CLOSE
  console.log('Channel,closeKey['+self.channelKey()+'],channel['+self.host+':'+self.port+']')
  self.emit('close')
  return callback(null, self._useState)
}
...
Channel.prototype.toKVProxy = function (callback) {
  ...
  var pKey = 'cp.' + sHostNum + KEY_SPLITOR + self.port
